...
testGroupIncludeFilesTinySet = TestCase (
  do
    ...
    let journals2 = [case Map.toList expected1 of
                       (x:_) -> fst x
                       []    -> error "expected1 should not be empty"] :: [TurtlePath]
    ...
    let journals3 = [case Map.toList expected2 of
                       (x:_) -> fst x
                       []    -> error "expected2 should not be empty"] :: [TurtlePath]
...
testIncludeYears = TestCase (
  do
    ...
    let actualerr = case map T.lines $ lefts [includeYears' txterr] of
                      (x:_) -> init x
                      []    -> error "Error message should not be empty" :: [Text]
